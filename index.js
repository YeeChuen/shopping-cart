...
const View = (() => {
  ...
  const createInventoryItem = (itemName, itemNumber, itemId) => {
    const divElem = createElement(
      "div",
      "inventory__item-container",
      itemId,
      undefined
    );
    const spanElem1 = createElement(
      "span",
    ...
    const btnElem1 = createElement(
      "button",
      "inventory__button-subtract",
      undefined,
      "-"
    );
    divElem.appendChild(btnElem1);
    const spanElem2 = createElement(
      "span",
      "inventory__item-number",
      undefined,
      itemNumber
    ); // <-- number is not yet implemented
    divElem.appendChild(spanElem2);
    const btnElem2 = createElement(
      "button",
      "inventory__button-add",
      undefined,
      "+"
    );
    divElem.appendChild(btnElem2);
    const btnElem3 = createElement(
      "button",
      "inventory__button-addcart",
      undefined,
      "add to cart"
    );
  ...
  const divPaginationContainer = document.querySelector(
  ...
  const renderInventory = (items, currentPage) => {
    ...
    items.forEach((e, i) => {
      if (
        ...
        ulInventoryElem.appendChild(
          createInventoryItem(e.content, e.number, e.id)
        );
      }
    });
...
const Controller = ((model, view) => {
  ...
  const handleUpdateAmount = () => {
    view.ulInventoryElem.addEventListener("click", (event) => {
      if (view.isInventoryAddBtn(event.target.classList[0])) {
        // handle add button
      ...
      } else if (view.isInventorySubtractBtn(event.target.classList[0])) {
        // handle subtract button
        const temp = [...state.inventory];
        temp.forEach((e, i) => {
          if (
            e.content == event.target.parentNode.firstChild.innerText &&
            e.number > 0
          ) {
            e.number--;
          }
  ...
  const handleAddToCart = () => {
    view.ulInventoryElem.addEventListener("click", (event) => {
      if (view.isInventoryAddCartBtn(event.target.classList[0])) {
        // handle add to cart button
        // if the item is not in cart.
        const inCart = state.cart.map((e) => e.content);
        const currId = event.target.parentNode.id;
        if (inCart.includes(event.target.parentNode.firstChild.innerText)) {
          // use PUT
          const addedItem = {};
          let newNumber = 0;
          ...
          // <-- here is if the item is not in cart, use POST
  ...
  const handleCheckout = () => {
    view.checkoutBtn.addEventListener("click", (event) => {
      ...
      /**
       * Pseudocode: if we want to have a loader after checkout is clicked,
       *              and the loader should disappear after checkout is done,
       *              how to implement that?
       * 
       * 1. remove innerHTML in the cart
       *
       * 2. add the HTML loader
       *
       * 3. await for the promise
       *
       * 4. remove the HTML loader
       *
       */
    });
  };
  ...
  const handlePagination = () => {
    view.divPaginationContainer.addEventListener("click", (event) => {
      ...
      if (
        view.isPaginationNextButton(event.target.classList[0]) &&
        currentPage < pageNum
      ) {
        currentPage++;
        const temp = [...state.inventory];
      ...
      if (
        view.isPaginationPrevButton(event.target.classList[0]) &&
        currentPage > 1
      ) {
        currentPage--;
        const temp = [...state.inventory];
